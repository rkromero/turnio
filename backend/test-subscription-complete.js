...
const TEST_CONFIG = {
  // Para pruebas rápidas: 1 minuto en lugar de 30 días
  testingMode: true,
  quickTestMinutes: 1, // Minutos para simular vencimiento
...
function showUsage() {
  ...
  console.log('');
  console.log('node test-subscription-complete.js [comando]');
  console.log('');
...
async function createTestSubscription() {
  ...
  try {
    ...
    if (TEST_CONFIG.testingMode) {
      // Para pruebas: vence en 1 minuto
    ...
    console.log('✅ Suscripción creada:', {
      ...
      testingMode: TEST_CONFIG.testingMode ? 'SÍ (vence en 1 min)' : 'NO (vence en 30 días)'
    });
...
async function simulateWebhook() {
  ...
  try {
    ...
    if (!subscription) {
      console.log('❌ No se encontró suscripción para simular webhook');
      return;
    }

    // Crear un pago simulado (representando cobro automático exitoso)
    const simulatedPayment = await prisma.payment.create({
      data: {
        ...
        paidAt: new Date(),
        mercadoPagoPaymentId: `webhook_payment_${Date.now()}`
      }
    });

    console.log('1️⃣ Pago automático simulado:', {
      id: simulatedPayment.id,
      amount: simulatedPayment.amount,
      status: simulatedPayment.status
    });

    // Actualizar suscripción (renovar por 30 días más)
    const nextBillingDate = new Date();
    nextBillingDate.setDate(nextBillingDate.getDate() + 30);
    ...
    console.log('2️⃣ Suscripción renovada:', {
      id: subscription.id,
      status: 'ACTIVE',
      nextBillingDate: nextBillingDate.toISOString()
    });

    console.log('\n✅ Cobro automático simulado exitosamente');
...
async function fullTestFlow() {
  ...
  try {
    ...
    console.log('\n⏳ Esperando vencimiento automático...');
    console.log(`Esperando ${TEST_CONFIG.quickTestMinutes} minuto(s)...`);
    console.log('(En producción serían 30 días)');
    ...
    console.log('✅ Suscripción creada');
    console.log('✅ Vencimiento procesado automáticamente');
    console.log('✅ Cobro automático simulado');
    console.log('✅ Suscripción renovada');
    console.log('');
    console.log('💡 Para limpiar datos: node test-subscription-complete.js cleanup');
